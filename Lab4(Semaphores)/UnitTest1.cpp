...
namespace unit_test1
{
    ...
    public:
        ...
        TEST_METHOD(TestMethod1)
        {
            ...
            {
                Sleep(80);
                SetEvent(events[mes[i]]);
                ResetEvent(events[mes[i]]);
                
            }

          
            Assert::IsTrue(mes.size() ==4);
        ...
        TEST_METHOD(TestMethod2)
        {
            ...
            STARTUPINFO si;
            PROCESS_INFORMATION pi{};
            std::string employee_cmd = "Employee.exe " + std::to_string(6) + " " + std::to_string(4);
            auto converting_employee_to_lpsz = std::wstring(employee_cmd.begin(), employee_cmd.end());
            const LPWSTR lpsz_employee_process_command_line = &converting_employee_to_lpsz[0];
            ZeroMemory(&si, sizeof(STARTUPINFO));
            si.cb = sizeof(STARTUPINFO);
            ...
            std::vector<int> mes = { 0,1,2,3,4,3 };
            std::vector<int> vec1;
